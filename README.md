...
A `pgsql:` DSN in `database/connections.php` is the whole switch — the driver
is picked from the connection (`Drivers/pgsql.php`), and models, relations,
eager loading, soft delete, pagination, the validator and the scheme cache all
answer as they do on MySQL. Enable `extension=pdo_pgsql` in php.ini.

```php
'pg' => ['pgsql:host=127.0.0.1;port=5432;dbname=app', 'app_user', 'secret', 'options' => [
...
What the driver translates so your code does not have to:

| you write | PostgreSQL gets |
|---|---|
| `limit(20, 10)` | `LIMIT 10 OFFSET 20` |
| `where('name', 'LIKE', '%ali%')` / `NOT LIKE` | `ILIKE` / `NOT ILIKE` — case-insensitive, as MySQL's collations make LIKE |
...
| migration `primary` | `PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY` |
| migration `onupdate` | a `BEFORE UPDATE` trigger keeping the column current (no ON UPDATE clause exists) |

`db migrate` speaks both dialects — add, modify (as `ALTER COLUMN TYPE` / `SET|DROP NOT
NULL` / `SET|DROP DEFAULT`), drop, indexes (`DROP INDEX`, identifiers quoted with `"`) and
`--fresh` all work; `charset:` flags, `$storageEngine` and column position (`AFTER`/`FIRST`)
are MySQL concepts and are ignored. What stays MySQL-only:
...
Name tables and columns in lower case (the convention throughout this
...
```bash
php terminal make mongomodel Post           # writes the skeleton above
php terminal mongo status                   # ping + server version
php terminal mongo indexes                  # creates every model's indexes(), re-runnable
```
...
php terminal make mongomodel Log --table=logs   # a MongoModel, see 2.9
...
# MongoDB (2.9)
php terminal mongo status
php terminal mongo indexes

# Tests (14.4)
php terminal tests
php terminal tests run db --db=local --filter=csrf
php terminal tests list
php terminal tests make posts
...
as it stands. Only `--db` and `--filter` reach the per-file process; other flags
are dropped. The shipped files (`db`, `validator`, `view`, `helpers`, `http`,
