        ...
        function createEndPointTester(parent) {
            $(function() {
                ...
                _PARENT.find('#method').on('input', function() {
                    ...
                    if (['GET', 'HEAD'].includes(this.value.toUpperCase())) _PARENT.find('[data-bs-target="#bodyTab"], #bodyTab').attr('disabled', true).css('opacity', .3).tooltip('dispose').tooltip({
                        title: '<small>Body can not use with GET or HEAD</small>',
                        html: true
                    });
                });
                ...
                function renderUrl() {
                    const $w = _PARENT.find('#urlWrapper').empty();
                    const re = /\{(\??)([^}]+)\}/g;
                    let li = 0,
                        m;
