...
- **`@yield` is resolved at compile time, not at runtime** (`View::parseYields()`). Sections are
...
- **`@extends($variable)` works and disables the view cache** for that template
  (`View::parseExtends()` sets `$hasDynamicExtends`; `render()` skips the cache on it). Every
  request recompiles. Avoid unless the layout genuinely varies.
...
- `@include` splices the file's text in and compiles it with the parent, max depth 32
  (`View::parseIncludes()`, `MAX_INCLUDE_DEPTH`). Nothing in this repo uses it; partials are called at runtime with
...
- Compiled output goes to `zFramework/storage/views/*.compiled.php` with an mtime manifest.
  Clear it with **`php terminal cache clear views`**. The command takes a fixed option list
  (`views|sessions|pages|ratelimit|logs`); a directory that does not exist yet clears 0.
- Caching and minify are both on by default (`framework.view`); turn caching off
...
are tried in order (`View::resolveViewPath()`):
...
Binds re-run on every request even when the compiled cache hits (`View::applyBinds()`, called
from the cache-hit branch of `render()` too).
